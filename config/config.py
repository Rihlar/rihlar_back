...
DiscordCallback = https://localhost:8943/auth/oauth/discord/callback

GoogleClientID = {google_client_id}
GoogleClientSecret = {google_client_secret}
GoogleCallback = https://localhost:8943/auth/oauth/google/callback

GithubClientID = {github_client_id}
GithubClientSecret = {github_client_secret}
GithubCallback = https://localhost:8943/auth/oauth/github/callback

MicrosoftClientID = {microsoft_client_id}
MicrosoftClientSecret = {microsoft_client_secret}
MicrosoftCallback = https://localhost:8943/auth/oauth/microsoftonline/callback
...
def main():
    ...
    # app.env のテンプレート
    session_secret_app = generate_random_key()
    app_env_template = f"""
SessionSecret = "{session_secret_app}"
...
"""
    ...
    # gcore.env のテンプレート
    session_secret_gcore = generate_random_key()
    redis_password_gcore = generate_random_key()
    gcore_env_template = f"""
SessionSecret = "{session_secret_gcore}"
GRPC_SERVER = auth:9000
DATABASE_DSN = "main:main@tcp(db:3306)/maindb?charset=utf8mb4&parseTime=True&loc=Local"

REDIS_ADDR = "redis:6379"
REDIS_PASSWORD = "{redis_password_gcore}"
"""
    # gcore.env ファイルを生成
    create_env_file("gcore.env", gcore_env_template)

    # game.env のテンプレート (app.env と同じ内容だが、キーは新規生成)
    session_secret_game = generate_random_key()
    game_env_template = f"""
SessionSecret = "{session_secret_game}"
GRPC_SERVER = auth:9000
DATABASE_DSN = "main:main@tcp(db:3306)/maindb?charset=utf8mb4&parseTime=True&loc=Local"
"""
    # game.env ファイルを生成
    create_env_file("game.env", game_env_template)

    # user.env のテンプレート (app.env と同じ内容だが、キーは新規生成)
